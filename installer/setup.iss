...
PasswordLabel3=الرجاء إدخال كلمة المرور الخاصة بالمثبت للمتابعة.%n%nإذا كنت لا تملك كلمة المرور، يرجى التواصل مع المطور.
...
[UninstallRun]
Filename: "{cmd}"; Parameters: "/c taskkill /f /im ProductApp.exe 2>nul"; Flags: runhidden
...
begin
  ...
  begin
    ...
    Sleep(500);
    Exec('cmd', '/c if exist "' + ExpandConstant('{app}') + '\inventory.db" del /f /q "' + ExpandConstant('{app}') + '\inventory.db"', '', SW_HIDE, ewWaitUntilTerminated, UninstallResult);
  end;
end;
